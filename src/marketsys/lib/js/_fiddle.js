function obtenerComprobanteFirmado_sri(id, ruta_certificado, pwd_p12, ruta_respuesta, nombre ,ruta_factura) {
    var response = [];
    $.ajax({
        url: "../facturaElectronica/app/leerFactura.php",
        type: 'POST',
        data: {
        ...
        oReq.onload = function (oEvent) {
            ...
            $.ajax({
                url: "../facturaElectronica/app/firma.php",
                type: 'POST',
                data: {
                ...
                $.ajax({
                    type: 'POST',
                    url: "../facturaElectronica/servicios/validarComprobante.php",
                    data: {
                        'id': id,
						'service': service, 
                    ...
                    if (/RECIBIDA/i.test(respuesta) || /CLAVE ACCESO REGISTRADA/i.test(respuesta)) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: "../facturaElectronica/servicios/autorizacionComprobante.php",
                            data: {
							...
							window.open('../reports/facturaHTML.php?idfactura='+id);
							location.reload();
						...
						//console.log(response);
						window.open('../reports/facturaPDF.php?idfactura='+id);
						location.reload();
                    }
...
function obtenerComprobanteFirmado_sri_rt(id, ruta_certificado, pwd_p12, ruta_respuesta, nombre ,ruta_factura,idRetenc) {
    var response = [];
    $.ajax({
        url: "../facturaElectronica/app/leerFactura.php",
        type: 'POST',
        ...
        oReq.onload = function (oEvent) {
            ...
            $.ajax({
                ...
                $.ajax({
                    type: 'POST',
                    url: "../facturaElectronica/servicios/validarComprobanteRt.php",
                    data: {
                        'id': id,
						'service': service, 
						'claveAcceso': $claveAcceso.text(),
                    ...
                    if (/RECIBIDA/i.test(respuesta) || /CLAVE ACCESO REGISTRADA/i.test(respuesta)) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: "../facturaElectronica/servicios/autorizacionComprobanteRt.php",
                            data: {
                                'service': service, 'claveAcceso': $claveAcceso.text()
                            },
                            context: document.body
							...
							if (document.getElementById('chkRetenciones').checked == true){			
								window.open('../reports/retencionPDF.php?id='+idRetenc);
							}
							//window.open('../reports/facturaPDF.php?idfactura='+id);
							location.reload();
...
function obtenerComprobanteFirmado_sri_rt_(id, ruta_certificado, pwd_p12, ruta_respuesta, nombre ,ruta_factura,idRetenc) {
    var response = [];
    $.ajax({
        url: "../facturaElectronica/app/leerFactura.php",
        type: 'POST',
                ...
                $.ajax({
                    ...
                    if (/RECIBIDA/i.test(respuesta) || /CLAVE ACCESO REGISTRADA/i.test(respuesta)) {
                        ...
                        $.ajax({
                            ...
                            response[1] = autorizacion_comprobante;
							//console.log(response);
							window.open('../reports/retencionPDF.php?id='+id);
							location.reload();
...
function fechas_certificado(ruta_certificado, mi_pwd_p12, ruta_respuesta) {
    ...
    oReq.onload = function (oEvent) {
        ...
        $.ajax({
            type: 'POST',
            url: "../facturaElectronica/app/validarFechaCertificado.php",
            data: {
...
function fechas_certificado_nw(ruta_certificado, mi_pwd_p12, tipo) {
	var ruta_respuesta;
    var response = [];
    ...
    oReq.onload = function (oEvent) {
        try {
			...
			$.ajax({
				...
				if (tipo == 1){
					resp = eval(respuesta);
					parent.document.getElementById('fechaVigencia').value 	= resp[2];
					parent.parent.modalAlertPrincipal(resp[0],
													 'MarketSys', resp[1], 0, 'Aceptar', '');
					if (resp[0] == 3){
						document.getElementById('imagenFormulario').src = '../images/icons/keyICON.png';
						parent.document.getElementById('validezFirma').value = 'S';
					}else{document.getElementById('imagenFormulario').src = '../images/icons/keyNotICON.png';
						  parent.document.getElementById('validezFirma').value = 'N';}
					
				}else if (tipo == 2){
					resp = eval(respuesta);
					document.getElementById('fechaVigencia').value 	= resp[2];
					parent.modalAlertPrincipal(resp[0],
											  'MarketSys', resp[1], 0, 'Aceptar', '');
				}	 
			});
		} catch (err) {
			if (tipo == 1){
				parent.document.getElementById('validezFirma').value = 'N';
				parent.parent.modalAlertPrincipal(1,
												 'MarketSys', err, 0, 'Aceptar', '');
				document.getElementById('imagenFormulario').src = '../images/icons/keyNotICON.png';
				}else{parent.modalAlertPrincipal(1,
												 'MarketSys', err, 0, 'Aceptar', '');
					  document.getElementById('imagenFormulario').src = '../images/icons/keyNotICON.png';
...
function validar_pwrd_nw(ruta_certificado, mi_pwd_p12) {
    var ruta_respuesta; 
    var oReq = new XMLHttpRequest();
    oReq.open("GET", ruta_certificado, true);
    oReq.responseType = "arraybuffer";
    oReq.onload = function (oEvent) {
        ...
        try {
            ...
            forge.pkcs12.pkcs12FromAsn1(p12Asn1, mi_pwd_p12);
			parent.parent.modalAlertPrincipal(3, 'MarketSys', 
												 'La contraseña y el certificado son correctos.', 0, 
												 'Aceptar', '');

        } catch (err) {
            parent.parent.modalAlertPrincipal(1, 'MarketSys', 
												 'La contraseña no es válida.', 0, 
												 'Aceptar', '');
        }
    }
    oReq.send();
