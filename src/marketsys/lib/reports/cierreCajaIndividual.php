		...
		$db = new MySQL();
 		$consulta = $db->consulta("select ap.id_apertura id_apertura,
								    ...
								    INNER JOIN tsc_aperturas_caja ap
									  ...
									  AND ap.estado IN('A') 
									--  AND DATE_FORMAT(p.fecha_pago,'%d-%m-%Y') = DATE_FORMAT(ap.fecha_apertura,'%d-%m-%Y')
								    INNER JOIN tgn_personal pr
									   ON pr.id_personal = p.id_personal
									  AND pr.id_personal = ap.id_personal
								      AND pr.id_personal = ".$_SESSION["idUsuario"]." 										   
								    where p.estado = 'A'
		...
		$db = new MySQL();
 		$consulta = $db->consulta("select f.numero_factura,	
									...
									INNER JOIN tsc_aperturas_caja ap
									  ...
									  AND ap.estado = 'A' 
									  AND DATE_FORMAT(f.fecha_registro,'%d%m%Y') = DATE_FORMAT(ap.fecha_apertura,'%d%m%Y')
									INNER JOIN tgn_personal pr
									   ON pr.id_personal = f.id_cajero
									  AND pr.id_personal = ap.id_personal 
								      AND pr.id_personal = ".$_SESSION["idUsuario"]." 										   
									where f.estado_electronico = 'A'
